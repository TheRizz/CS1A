...
int main()
{
	...
	PrintHeader(cout, "Implementing a Queue", 'L', 10);

	// PROC - do while loop for menu prompt until user inputs exit key
	do{
		menuChoice = MenuPrompt();

		if(IsEmpty(head) && menuChoice != ENQUEUE && menuChoice != EXIT)
		{
		cout << "\nThe QUEUE is empty!\n\n";
		}
		else
		...
		switch(menuChoice)
		{
		case ENQUEUE : Push(head);
		break;
		case DEQUEUE : Pop(head);
		break;
		...
		break;
		case FRONT: Peek(head);
		break;
		case SIZE: queueSize = Size(head);
		if(queueSize == 1)
		{
		...
		break;
		case CLEAR_QUEUE: ClearQueue(head);
						  cout << endl;
		break;
